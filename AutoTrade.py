...
for i, code in enumerate(codeList):
    if objCpCodeMgr.GetStockSectionKind(code) == 1 :
        ...
        if codeCount < 75 and stdPrice <15000:
...
def get_kvalue(code):  # 인자로 받은 종목에 대한 20일 average noise ratio.
    try:
        ohlck = get_ohlc(code, 7)
        ohlck['noiseratio'] = 1 - (abs( ohlck['open']-ohlck['close'] ) / (ohlck['high'] - ohlck['low']))
        # printlog('open : ', ohlck['open'])
        # printlog('close : ', ohlck['close'])
        # printlog('high : ', ohlck['high'])
        # printlog('low : ', ohlck['low'])
...
def get_target_price(code):
    ...
    try:
        ...
        if k==0 :
            k=0.5
        target_price = today_open + (lastday_high - lastday_low) * (k * 0.6)
        
        printlog(code, countCount, bought_list)
        
        # printlog('today_open : ', today_open)
        # printlog('lastday_high : ', lastday_high)
        # printlog('lastday_low : ', lastday_low)
        # printlog('target_price : ', target_price)
        # printlog('k : ', k)
        # printlog('-----------------------------')
...
def buy_etf(code):
    ...
    try:
        ...
        global bought_price_list
        # printlog('code:', code, '의 buy_etf 실행중')
        ...
        # printlog('current_price :', current_price, ', target_price :', target_price,', ma5_price :',ma5_price,', ma10_price :',ma10_price)
        if current_price > target_price and current_price > ma5_price \
            ...
            if bought_qty > 0:
                ...
                bought_price_list.append(current_price)
                symbol_list.remove(code)
                printlog('bought_list :', bought_list)
    ...
    try:
        ...
        if stock_qty != 0:                  
           ...
           # 최유리 IOC 매도 주문 요청
            ...
            bought_price_list.pop(bought_list.index(code))  # index로 나온건 숫자 1
...
if __name__ == '__main__': 
    try:
        # symbol0 = 'A005090' #SGC에너지
        # symbol1 = 'A030790' #비케이탑스
        # symbol2 = 'A271980' #제일약품
        # symbol3 = 'A005870' #휴니드
        # symbol4 = 'A010820' #퍼스텍
        # symbol5 = 'A010600' #웰바이오텍
        # symbol6 = 'A137310' #에스디바이오센서
        # symbol7 = 'A105840' #우진
        # symbol8 = 'A372910' #한컴라이프케어
        # symbol9 = 'A145720' #덴티움
        # symbol10 = 'A003160' #디아이
        # symbol_list = [symbol0,symbol1,symbol2,symbol3,symbol4,symbol5,symbol6,symbol7,symbol8,symbol9,symbol10]
        
        bought_list = ['A001250','A002350','A002070','A001470','A000520','A000400']     # 매수 완료된 종목 리스트
        bought_price_list = [2355,6900,2935,2230,8390,1605]
        ...
        while True:
            ...
            #             printlog('bought_list', bought_list)
            #             current_price, ask_price, bid_price = get_current_price(bou) 
            #             index=bought_list.index(bou)
            #             if current_price > bought_price_list[index] * 1.008:
            #                 sell_one(bou)
            #                 printlog('----sdgfkjisdfjiswpfjwefwefewsf')
            #                 time.sleep(1)
            #!
            t_now = datetime.now()
            t_9 = t_now.replace(hour=9, minute=0, second=0, microsecond=0)
            t_start = t_now.replace(hour=9, minute=1, second=0, microsecond=0)
            t_sell = t_now.replace(hour=15, minute=14, second=0, microsecond=0)
            ...
            if t_start < t_now < t_now.replace(hour=15, minute=0, second=0, microsecond=0) :  # AM 09:05 ~ PM 01:20 : 매수
                for sym in symbol_list:
                    if len(bought_list) < target_buy_count:
                        buy_etf(sym)
                        # printlog('-----------------------------')
                        countCount+=1
                        time.sleep(1.2)
                if t_now.minute == 30 and 0 <= t_now.second <= 59: 
                    get_stock_balance('ALL')
                    time.sleep(5)
            #개별 매도
            if t_9 < t_now < t_now.replace(hour=15, minute=15, second=0,microsecond=0):
                for bou in bought_list:
                    current_price, ask_price, bid_price = get_current_price(bou) 
                    index=bought_list.index(bou)
                    if current_price > bought_price_list[index] * 1.008 or current_price < bought_price_list[index] * 0.992:
                        sell_one(bou)
                        time.sleep(1)
